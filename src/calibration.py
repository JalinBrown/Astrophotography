...
base_dir = Path('./data')
raw_dir = base_dir / 'raw_data' # Create dir for raw photos with folder of the frame types
...
def create_master_bias():
    ...
    master_bias = ccdp.combine(
        files_to_combine, method='median', unit=u.adu,
        mem_limit=2.5e9,
...
def create_master_dark(master_bias):
    ...
    if len(exp_times) == 0:
        ...
        print(f"Error: No '{EXPOSURE_KEY}' keyword found in any FITS files in {fits_dark_dir}.")
        print("This means the 'rawpy' conversion failed to get exposure times.")
        sys.exit()
    
    if len(exp_times) > 1:
        print(f"\n\nCRITICAL ERROR in 'create_master_dark':")
        print(f"Error: Your dark frames have multiple different exposure times: {exp_times}")
        print("A Master Dark can only be created from darks with the *exact same* exposure time.")
        sys.exit()
    ...
    master_dark = ccdp.combine(
        dark_frames_to_combine, method='median', unit=u.adu,
        mem_limit=2.5e9, sigma_clip=True,
        sigma_clip_low_thresh=5, sigma_clip_high_thresh=5
    )
    
    master_dark.header[EXPOSURE_KEY] = (dark_exposure_time.value, 'Exposure time of master dark')
...
def create_master_flat(master_bias, master_dark):
    ...
    master_flat = ccdp.combine(
        flat_frames_to_combine, method='median', unit=u.adu,
        mem_limit=2.5e9, sigma_clip=True,
        sigma_clip_low_thresh=5, sigma_clip_high_thresh=5
    )
...
if __name__ == '__main__':
    ...
    for d in [fits_bias_dir, fits_dark_dir, fits_flat_dir, fits_light_dir, master_dir, calibrated_dir]:
